    ...
    nodes: {
      q_new: {
        ...
        opts: [
          ...
          { t: "Non — marchand existant (déjà enregistré)", tone: "ok", go: "r_existing" },
          { t: "Non — hors périmètre DAC7 (US, UAE…)", tone: "ok", go: "r_na" },
        ],
      },
      ...(lob === "travel" ? {
        q_au: {
      ...
      r_existing: { result: {
        decision: "pass", tag: "—",
        internal: "Marchand existant — DAC7 OK.",
        ecw: "", action: "Aucune action DAC7 requise.",
      }},
      r_na: { result: {
        decision: "pass", tag: "—",
        internal: "Hors périmètre DAC7 (US / UAE).",
        ecw: "", action: "Aucune action DAC7 requise.",
      }},
    },
  };
...
// --- PRICING ---
// Noeud générique de contrôle du format (PDF/PNG/JPEG) ; en cas de succès → `next`.
function fmtNode(next) {
  return {
    q: "Le document PV est-il dans un format accepté ? (.pdf, .png ou .jpeg uniquement — pas de .doc/.xlsx/.txt ni d'éditable)",
    opts: [
      { t: "Oui — PDF / PNG / JPEG", tone: "ok", go: next },
...
// Renvoie un objet de nodes à fusionner. entry = "pvv_type" (on demande d'abord le type de PV).
...
function pvValidityNodes(market, okGo, isGoods) {
  ...
  return {
    pvv_type: {
      q: "Quel type de PV ?",
      help: <>On identifie d'abord la nature de la preuve de prix, puis on vérifie le format (PDF/PNG/JPEG) et les conditions propres à ce type.</>,
      opts: [
        { t: "Capture du site marchand / concurrent (ex. Treatwell)", go: "pvv_fmt_screenshot" },
        { t: "Reçu ou facture", go: "pvv_fmt_receipt" },
        { t: "Menu / liste de prix (price list)", go: "pvv_fmt_pricelist" },
        { t: "Manuscrit (dernier recours)", tone: fr ? "danger" : "warn", go: fr ? "rpv_handwritten_fr" : "pvv_fmt_owner" },
      ],
    },
    // Vérification du format (commune), branchée vers le contrôle propre à chaque type
    pvv_fmt_screenshot: fmtNode("pvv_screenshot"),
    pvv_fmt_receipt: fmtNode("pvv_receipt"),
    pvv_fmt_pricelist: fmtNode("pvv_pricelist"),
    pvv_fmt_owner: fmtNode("pvv_owner"),
    pvv_screenshot: {
      q: "La capture respecte-t-elle TOUTES ces conditions : moins de 30 jours, horodatage (timestamp) visible ET URL visible ?",
      help: <>Une capture de site n'est valable comme PV que si on peut <b>tout vérifier</b> : la date (moins de 30 jours), un <b>horodatage</b> visible (date/heure de la capture) et l'<b>URL</b> de la page. Sans ces éléments, impossible de prouver que c'est bien le prix actuel du marchand.<br /><br />Cas fréquent : la capture jointe par Sales est absente ou fausse, <b>mais le bon prix est sur le site</b> → c'est à toi (agent CO) de reprendre la capture et de l'attacher à l'opportunité.</>,
      opts: [
        { t: "Oui — moins de 30 jours, avec horodatage et URL", tone: "ok", go: "pvv_owner" },
...
function pricingTree(lob, market) {
  ...
  const convNote = (lob === "local" || lob === "glive" || lob === "national")
    ? " Devise étrangère : convertir via Google Converter en devise locale (±5 % toléré), avec capture attachée à l'opportunité."
    : "";

  // SOP par variante
  const sop = isGoods
    ? <>Goods : PV requis dès qu'il y a une remise. PV valide = URL vendeur/fabricant, image, prix, achetable. la Unit Value SF ne doit pas dépasser le prix du PV. Liste de prix : <b>{priceListMonths(market)} mois</b> max sur ce marché (DE/ES 6, autres 12). UK + marque T1 Comparable → Reputable Retailer/fabricant (refurb : PV vendeur OK).</>
    : isTravel
    ...
    : isGlive
    ? <>GLive : tenter de valider (site ou PV ; LiveNation Tour Docs acceptés). Si validable → notre prix ne doit pas dépasser celui du site. Si non validable / PV non conforme → <b>pousser sans IR</b>.{convNote}</>
    : isMetro
    ? <>Metro : prix vérifié via le lien fourni par le marchand. Si le service est listé mais que notre prix est supérieur à celui du site → amender la valeur. Si l'ajustement crée une remise négative → Follow-Up « Incorrect Pricing » (les DEUX prix dans l'ECW). Service non listé / pas de prix / site KO → approuver (US) ou approuver avec remise masquée (INTL).</>
    : nam
    ? <>NAM &amp; CA Local : <b>PV requis si la remise est supérieure ou égale à 40 %</b>. Comparer site (et amendement si « Signed with the contract ») vs SF. Site inférieur à SF → <b>IR</b> même si « Signed with the contract ». Pas de prix site → champ Proof of Pricing en SF.{convNote}</>
    : <>Local / National (INTL) : <b>pas de seuil de 40 %</b>. Toujours tenter de valider via le site ou un PV. Comparer le prix public au prix SF (et au prix de l'amendement de prix signé le cas échéant). Prix du site inférieur à SF → <b>IR</b>. Pas de prix public → champ Proof of Pricing en SF.{convNote}</>;
  ...
  const nodes = {
    ...
    q_extranet: {
      ...
      opts: [
        { t: "Oui — un PV par add-on", tone: "ok", go: "pvv_type" },
    ...
    q_voucher: {
      ...
      opts: [
        { t: "Oui — nuitée + add-ons couverts", tone: "ok", go: "pvv_type" },
    ...
    q_prop: {
      q: "Le prix Groupon est-il inférieur ou égal au prix du site (hôtel) ?",
      opts: [
        { t: "Oui, prix inférieur ou égal au site", tone: "ok", go: "r_pass" },
    ...
    q_glive: {
      ...
      opts: [
        ...
        { t: "Oui — via un document PV", go: "pvv_type" },
    ...
    q_goods_pv: {
      q: "Un PV est-il attaché ET la Unit Value SF est-elle inférieure ou égale au prix du PV ?",
      opts: [
        { t: "Oui — PV présent, Unit Value inférieure ou égale au PV", tone: "ok", go: "pvv_type" },
        { t: "PV manquant, ou Unit Value supérieure au prix du PV", tone: "warn", go: "r_ir_goods" },
      ],
    },
    r_ir_goods: { result: { decision: "ir", tag: "—", internal: "Pricing Goods : PV manquant, ou Unit Value supérieure au prix du PV. IR. Rappel : Amazon OK si « Dispatched and sold by Amazon » ou nom vendeur = SF ; eBay seulement si boutique propre ; Allegro/AliExpress/convertisseurs refusés.", ecw: "Merci de fournir un Proof of Pricing valide (URL vendeur/fabricant, image produit, prix, produit achetable) ; la valeur du deal ne peut pas dépasser le prix justifié.", action: "IR à Sales." }},

    // ---------- METRO (Global) ----------
    q_metro: {
      q: "Le service exact est-il listé sur le site du marchand, et comment se compare le prix ?",
      opts: [
        { t: "Listé — notre prix est supérieur à celui du site", tone: "warn", go: "q_metro_neg" },
        { t: "Listé — prix correct ou notre prix inférieur ou égal au site", tone: "ok", go: "r_pass" },
    ...
    r_metro_amended: { result: { decision: "ir", tag: "—", internal: "Pricing Metro : service listé et notre prix supérieur à celui du site → valeur amendée au prix du site. Capture attachée, IR Account Owner, #valueadjusted en Context Notes. " + drinkRule, ecw: "", action: "Amender SF → capture → attacher → IR Account Owner → #valueadjusted." }},
    ...
    q_metro_like: {
      ...
      opts: [
        { t: "Non — notre prix inférieur ou égal au site", tone: "ok", go: "r_pass" },
        { t: "Oui — notre prix est supérieur à celui du site", tone: "warn", go: "r_metro_amended" },
      ],
    },
    ...
    q_local_site_lowdisc: {
      ...
      opts: [
        ...
        { t: "Site inférieur à SF (ou inférieur au prix de l'amendement de prix signé)", tone: "warn", go: "r_ir_local" },
      ],
    },
    q_local_site: {
    ...
    q_local_compare: {
      ...
      opts: [
        { t: "Oui — tout correspond", tone: "ok", go: "pvv_type" },
        { t: "Prix sur site inférieur à SF, OU inférieur au prix de l'amendement de prix signé", tone: "warn", go: "r_ir_local" },
      ],
    },
    ...
    r_ir_pvfield: { result: { decision: "ir", tag: "—", internal: "Pricing : pas de prix public et le champ PV n'est ni « Merchant Will Provide Later » ni « Signed with the contract ». IR demandant un Proof of Pricing.", ecw: "Un Proof of Pricing est requis pour valider le prix de ce deal. Merci de le fournir.", action: "IR — demander le PV." }},
    ...
    nodes: {
      q_mcl: {
        ...
        opts: [
          ...
          { t: "La MCL exige un document / une licence (diplôme, autorisation préfectorale…)", go: "q_doc" },
      ...
      q_doc: {
        q: "Le(s) document(s) exigé(s) par la MCL est-il (sont-ils) fourni(s) et valide(s) ?",
        help: <>La MCL peut exiger un document précis (diplôme, autorisation préfectorale, agrément…). Vérifiez d'abord <b>s'il est joint au deal</b> et <b>valide</b> : il peut très bien l'être. Si oui, on continue. S'il est joint mais invalide/expiré/incorrect, ou s'il manque, on le signale.</>,
        opts: [
          { t: "Oui — document(s) fourni(s) et valide(s)", tone: "ok", go: "r_docok" },
          { t: "Fourni mais invalide / expiré / incorrect", tone: "warn", go: "r_docinvalid" },
          { t: "Non fourni — le marchand doit le fournir", tone: "warn", go: "r_followdoc" },
          { t: "Non fourni — le Sales doit le transmettre", tone: "warn", go: "r_irdoc" },
        ],
      },
      ...
      r_docok: { result: { decision: "pass", tag: "—", internal: "MCL : document(s) exigé(s) fourni(s) et valide(s). Conforme.", ecw: "", action: "Continuer." }},
      r_docinvalid: { result: { decision: "follow", tag: "Provided Licensing is not the Correct Licensing", internal: "MCL : document exigé fourni mais invalide / expiré / incorrect. Follow-Up.", ecw: "Le document fourni au titre des exigences réglementaires n'est pas valide (invalide, expiré ou incorrect). Merci de fournir une copie valide du document requis.", action: "Follow-Up — document MCL invalide (choisir le tag licence approprié)." }},
      ...
      r_followdoc: { result: { decision: "follow", tag: "Required Licensing is Missing", internal: "MCL : document/licence exigé non fourni, à demander au marchand. Follow-Up.", ecw: "Un document complémentaire est requis pour valider ce deal. Merci de fournir [document requis par la MCL].", action: "Follow-Up — tag licence/document approprié." }},
      r_irdoc: { result: { decision: "ir", tag: "Legal", internal: "MCL : document exigé non fourni, détenu/transmis par Sales. IR à Sales.", ecw: "Un document complémentaire (détenu côté Sales) est requis pour valider ce deal.", action: "IR à Sales — document MCL." }},
...
function fraudTree(lob) {
  return {
    ...
    sop: <>Étape <b>finale</b> du vetting, après site web, licences, contenu et prix. Ouvrez le Gem « Merchant Fraud Screening » (bouton ci-dessous), renseignez nom / site / pays / titre du deal depuis Salesforce, puis lisez la <b>première ligne</b> du résultat. L'IA <b>ne remplace pas</b> le vetting manuel et ne sert <b>jamais</b> à rejeter automatiquement.</>,
    start: "q_fraud",
    nodes: {
      q_fraud: {
        q: "Quel est le résultat renvoyé par Gemini (première ligne) ?",
        link: { url: "https://gemini.google.com/gem/33dcc80790e2", label: "Ouvrir le Gem Merchant Fraud Screening" },
        help: <>Ouvrez le Gem (bouton ci-dessus) et renseignez : nom du marchand, site, pays, titre du deal. Le résultat commence <b>toujours</b> par l'une de deux lignes : « NO MAJOR FRAUD INDICATORS DETECTED » ou « MANUAL REVIEW REQUIRED ». On ne se fie qu'à cette ligne d'en-tête. Important : ne jamais rejeter un deal sur la seule base de l'IA.</>,
        opts: [
...
function goodsTree() {
  return {
    ...
    sop: <>Brand tier : T1 = scrutin max, T2/T3 ou vendeur exempté = vetter. <b>Apple</b> a un flux dédié (liste de vendeurs approuvés). Refurbished : grade A/B/C obligatoire ; si manquante → IR à Sales (contrairement à NAM Gateway qui rejette). Vendeurs exemptés : BVG Airflo, James Russell, E.com Intl (Home &amp; Garden), UPGS, Van Meuwen.</>,
    start: "q_brand",
    nodes: {
      q_brand: {
        ...
        opts: [
          { t: "Non — produit sans marque", tone: "ok", go: "q_refurb", frag: "Non-branded" },
          { t: "Marque absente du registre (Tier Brands sheet)", tone: "ok", go: "q_refurb", frag: "Marque hors registre" },
          { t: "Oui — c'est de l'Apple", go: "q_apple" },
          { t: "Oui — marque T2", tone: "ok", go: "q_refurb", frag: "Marque T2" },
          { t: "Oui — marque T3", tone: "ok", go: "q_refurb", frag: "Marque T3" },
          { t: "Oui — vendeur exempté", tone: "ok", go: "q_refurb", frag: "Vendeur exempté" },
      ...
      q_apple: {
        ...
        opts: [
          { t: "Oui — vendeur approuvé", tone: "ok", go: "q_airpods", frag: "Apple, vendeur approuvé" },
      ...
      r_hold_apple: { result: { decision: "hold", tag: "Legal", internal: "Apple : vendeur non approuvé → Legal hold (revue de provenance avec Ops).", ecw: "Une vérification de provenance est en cours pour ce produit de marque. Merci de fournir la documentation fournisseur (factures / chaîne d'approvisionnement) via votre commercial.", action: "Legal hold + IR à Sales pour provenance. Ne pas vetter tant que Legal n'a pas validé." }},
      r_ip: { result: { decision: "escalate", tag: "Legal", internal: "Produit « lookalike » Apple (non fabriqué par Apple) → flag Legal (infraction IP possible).", ecw: "", action: "Escalader à Legal (IP). Ne pas vetter comme produit Apple." }},
      q_t1: {
        ...
        opts: [
          { t: "Oui — approbation ou provenance présente", tone: "ok", go: "q_refurb", frag: "Marque T1 (approbation/provenance OK)" },
          { t: "Produit refurbished (provenance non requise)", tone: "ok", go: "q_refurb", frag: "Marque T1" },
          { t: "Non — ni approbation ni provenance", tone: "warn", go: "r_hold_t1" },
        ],
      },
      r_hold_t1: { result: { decision: "ir", tag: "Legal", internal: "Marque T1 : ni approbation ni document de provenance. Deal en attente.", ecw: "Ce produit de marque nécessite une validation interne (approbation ou document de provenance) avant mise en ligne.", action: "Obtenir une approbation d'un approbateur autorisé OU un provenance doc. IR à Sales si nécessaire." }},
      q_refurb: {
        ...
        opts: [
          ...
          { t: "Oui — grade A", tone: "ok", go: "q_valid", frag: "refurbished grade A" },
          { t: "Oui — grade B", tone: "ok", go: "q_valid", frag: "refurbished grade B" },
          { t: "Oui — grade C", tone: "ok", go: "q_valid", frag: "refurbished grade C" },
          { t: "Oui — grade MANQUANTE", tone: "warn", go: "r_refurb_ir" },
        ],
      },
      r_refurb_ir: { result: { decision: "ir", tag: "—", internal: "Produit reconditionné : grade A/B/C manquante. IR à Sales (contrairement à NAM Gateway qui rejette) ; body copy doit mentionner la grade avant lancement.", ecw: "Ce produit reconditionné doit afficher son grade de reconditionnement (A, B ou C). Merci de confirmer le grade auprès du fournisseur.", action: "IR à Sales pour la grade. Deal en Rep Getting Info." }},
      q_valid: {
      ...
      r_valid_ir: { result: { decision: "ir", tag: "—", internal: "Catégorie en scope de validation (Food/Cosmétique/Détergent) : check QA échoué. IR (QA matrix).", ecw: "Une documentation complémentaire est requise pour cette catégorie de produit. Merci de la fournir via votre commercial.", action: "IR à Sales (voir QA check matrix)." }},
      r_pass: { result: { decision: "pass", tag: "—", internal: "Conforme.", compose: true, ecw: "", action: "Continuer. Vérifier la shipping proof si cross-border ; ignorer les attributs MRT." }},
    },
  };
}
...
function buildChecks(lob, market) {
  const a = APPLIES[lob];
  const checks = [];
  // Ordre = trame Local/Metro : DAC7 → PDS Eligibility → Licences → MCL → Localisation
  //         → Pricing → Site web → Goods → Gemini (fraude). Les LoB hors Local/Metro
...
/* =========================================================================
   ...
   ========================================================================= */
function TextInputNode({ node, initialValue, onSubmit }) {
...
function CheckRunner({ tree, market, onDone }) {
  ...
  const [pdsPicked, setPdsPicked] = useState(null); // objet {id,label} pour réaffichage au retour
  const [priceCheck, setPriceCheck] = useState(""); // Price check name (Goods)
  const [frags, setFrags] = useState([]); // fragments de constat accumulés le long du parcours
  const node = tree.nodes[nodeId];

  // si on tombe sur un result, on remonte au parent (en y joignant PDS / Price check / fragments)
  React.useEffect(() => {
    if (node && node.result) {
      let res = node.result;
      // constat composé à partir des choix réels de l'agent (ex. "Marque T3, refurbished grade B : conforme.")
      if (res.compose && res.decision === "pass" && frags.length) {
        res = { ...res, internal: frags.map((f) => f.frag).join(", ") + " : conforme." };
      }
      if (pdsLabel) res = { ...res, internal: "PDS : " + pdsLabel + ". " + res.internal };
      if (priceCheck && nodeId === "r_pv_ok") res = { ...res, priceCheck };
      onDone(res, trail);
    }
  ...
  const pick = (o) => {
    if (o.frag) setFrags((prev) => [...prev.filter((f) => f.at !== nodeId), { at: nodeId, frag: o.frag }]);
    goTo(o.go);
  };
    ...
    <div>
      ...
      {node.lookup ? (
        <LvgLookup
          node={node}
          market={market}
          initialPicked={pdsPicked}
          onSelect={(label, target, pickedObj) => { setPdsLabel(label); if (pickedObj) setPdsPicked(pickedObj); goTo(target); }}
        />
      ) : node.input ? (
        <TextInputNode
          node={node}
          initialValue={node.field === "pricecheck" ? priceCheck : ""}
        ...
        <div style={{ marginTop: 22 }}>
          ...
          {node.link && (
            ...
            >🔗 {node.link.label} <span style={{ opacity: 0.7, fontSize: 12.5 }}>(nouvel onglet)</span></a>
          )}
          {node.opts.map((o, i) => (
            <Choice key={i} tone={o.tone} onClick={() => pick(o)}>
              {o.t}
            </Choice>
          ))}
        </div>
      ...
      {trail.length > 0 && (
        <button
          onClick={() => {
            const t = [...trail]; const prev = t.pop(); setTrail(t); setNodeId(prev);
            // on retire le fragment éventuellement choisi à ce noeud (il sera re-choisi en avançant)
            setFrags((f) => f.filter((x) => x.at !== prev));
          }}
          style={{
...
function LvgLookup({ node, market, initialPicked, onSelect }) {
  const [query, setQuery] = useState("");
  const [picked, setPicked] = useState(initialPicked || null); // {id,label}
      ...
      {!picked && (
        <div>
          ...
          <div style={{ marginTop: 14, display: "flex", gap: 10, flexWrap: "wrap" }}>
            <button onClick={() => onSelect("", node.goNotFound, null)}
            ...
            <button onClick={() => onSelect("DNC", node.goDnc, null)}
        ...
        <div style={{ marginTop: 14 }}>
          ...
          {req && req.text ? (
            <div style={{ marginTop: 10 }}>
              ...
              <Choice tone="neutral" onClick={() => onSelect(picked.label, node.goMassage, picked)}>
                C'est un PDS <b>Massage</b> → vérifier licence + Online Reviews + Location
              </Choice>
              <Choice tone="neutral" onClick={() => onSelect(picked.label, node.goWithReq, picked)}>
            ...
            <div style={{ marginTop: 10 }}>
              ...
              <Choice tone="ok" onClick={() => onSelect(picked.label, node.goNoReq, picked)}>
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{ marginTop: 8 }}>
          <Row h="Quand un PV est-il requis ?">
            NAM &amp; CA Local (non-Metro) : {hl("remise supérieure ou égale à 40 %")}. Getaways &amp; National : dès qu'il y a une remise.
            Travel Booking / Hotel Trader : {hl("pas de PV")}. Extranet : un PV par add-on inclus. Voucher : nuitée + chaque add-on.
            GLive : tenter ; sinon pousser sans IR. Goods : dès qu'il y a une remise (Unit Value inférieure ou égale au PV).
          </Row>
          <Row h="Validité d'un screenshot de site">
            {hl("moins de 30 jours")} + {hl("timestamp")} + {hl("URL")} visibles. Si capture absente/incorrecte mais bon prix sur le site → l'agent CO prend lui-même la capture et l'attache.
          </Row>
          <Row h="Autres justificatifs acceptés">
            Reçu / facture {hl("moins de 3 mois")} · Menu / liste de prix (pas d'expiration pour les services ; {hl("Goods : DE/ES 6 mois, autres 12 mois")}) · Price breakdown pour les packages (hors Goods) · LiveNation Tour Docs (NAM GLive).
