  ...
  return {
    ...
    nodes: {
      q_brand: {
        ...
        opts: [
          { t: "Non — produit sans marque", tone: "ok", go: "q_refurb", note: "Non-branded." },
...
function CheckRunner({ tree, market, onDone }) {
  ...
  const [noteOverride, setNoteOverride] = useState(""); // constat court imposé par une option (ex. Non-branded)
  const node = tree.nodes[nodeId];

  // si on tombe sur un result, on remonte au parent (en y joignant PDS / Price check / note imposée)
  React.useEffect(() => {
    if (node && node.result) {
      let res = node.result;
      if (noteOverride && res.decision === "pass") res = { ...res, internal: noteOverride };
  ...
  const pick = (o) => { if (o.note) setNoteOverride(o.note); goTo(o.go); };
